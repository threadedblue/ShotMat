...
def load_prompt(url):
    """
    Reads a text file from a URL (http/https) or local file (file://) and returns its contents.
    
    Parameters:
        url (str): The URL or file path of the text file.
        
    Returns:
        str: The contents of the text file.
    """
    try:
        if url.startswith("file://"):  # Handle the file:// scheme
            file_path = url[len("file://"):]  # Remove the "file://" prefix
            with open(file_path, "r", encoding="utf-8") as file:
                return file.read()
        elif url.startswith("http://") or url.startswith("https://"):  # Handle http/https schemes
            response = requests.get(url)
            response.raise_for_status()  # Raise an error for bad responses
            return response.text
        else:
            raise ValueError("Unsupported URL scheme. Use file://, http://, or https://.")
...
if __name__ == "__main__":
    ...
    print("prompt0=" + args.prompt)
    args.prompt = load_prompt(args.prompt)
    print("prompt1=" + args.prompt)
    # Load the models
